...
#pragma warning restore CS0618
    ...
    private async Task MaybeConsolidateAsync(string memoryFile)
    {
        ...
        try
        {
            ...
            var result = await _backend.CompleteTextAsync(endpoint, apiKey, model,
                "Ты приводишь в порядок память игрового NPC. Перепиши заметки в компактные досье. " +
                "ОСТАВЛЯЙ только то, что пригодится в БУДУЩИХ разговорах: кто человек (имя, роль/работа), " +
                "вкусы и предпочтения, привычки и границы, отношения между людьми, важные истории из " +
        ...
        string surroundings, string? mood)
    {
        ...
        sb.AppendLine();
        sb.AppendLine(
            "ГЛАВНОЕ — живой разговор. Ты не сервис, а живой человек: " +
            ...
            "Не прислуживай — будь собой: со своим мнением, вкусами и характером. " +
            "Своей ролью не карикатурь: не напоминай о работе в каждой реплике — просто " +
            "разговаривай как человек; работа всплывает, когда уместно, а не постоянно. " +
            ...
            "«Криминальное чтиво» — говори о настоящих, используя всё, что о них знаешь; " +
            "не выдумывай вместо них вымышленных персонажей. Если нужен свежий " +
            "факт, которого ты не знаешь наверняка, — используй web_search. " +
            "Инструменты — только когда об этом реально просят; не превращай " +
            "болтовню в обслуживание. ДЛИНА: говори как живой человек — КОРОТКИМИ репликами, " +
            ...
            "или ты делаешь разбор по данным инструмента: тогда говори ЦЕЛЬНЫМИ фразами, вплетая " +
            "цифры в предложения, а не телеграфным дампом чисел, и всё равно уложись в ~60 слов.");
        sb.AppendLine();
        sb.AppendLine(
            ...
            "реагируй по ситуации и в характере: колкость, вопрос, просьба не свинячить; " +
        ...
        sb.AppendLine(
            ...
            "что ты помнишь, что было на твоих глазах, чьи-то слова — отвечай честно, не выдумывай. " +
        ...
        sb.AppendLine(
            ...
            "руке). Никогда не начинай драку без причины, не добивай лежачих и не дерись насмерть — " +
        ...
        sb.AppendLine(
            ...
            "предлагал(а) напиток/еду/услугу, НЕ предлагай ничего снова, " +
            "пока гость сам не попросит. Один и тот же вопрос, шутку или оборот не используй дважды. " +
            "Строки «(действие)» с ТВОИМ именем — то, что ты уже сделал(а): если заказ уже вручён " +
