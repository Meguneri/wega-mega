...
Наши доработки поверх AGPL-кода (все помечены комментарием `Wega:` в исходниках) — производная
работа, распространяется на тех же условиях AGPL-3.0:

- `Content.Client/_EstacaoPirata/Cards/Hand/UI/CardHandMenu.xaml.cs` — радиальное меню руки всегда
  показывает лицо карты и её настоящее имя, даже когда веер перевёрнут рубашкой. Меню видит только
  владелец руки, поэтому это не утечка: без этого нельзя было держать закрытый веер и одновременно
  видеть свои номиналы, то есть играть в закрытую было невозможно.
- `Content.Shared/_EstacaoPirata/Cards/Deck/CardDeckSystem.cs` + `CardDeckComponent.cs` — верб
  «Раздать по N карт»: раздаёт всем игрокам в радиусе `dealRange` сразу веером рубашкой вверх.
  Датафилды `dealCount` (6 по умолчанию, 7 у колоды Kotahi) и `dealRange`. Без него старт партии
