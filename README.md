...
Кратко о персистентности и личных базах:

- **Прогресс сохраняется.** Стэш, баланс телекристаллов и статистика рейдера хранятся в локальной БД сервера (`RaidStashSystem`) и восстанавливаются при коннекте игрока.
- **У каждого игрока своя база.** При коннекте для аккаунта создаётся отдельная карта `Resources/Maps/_Wega/Raid/hideout.yml` со своим гридом, личным схроном (`RaidStashBox`) и магазином (`RaidShopTerminal`).
- **Телепорт на базу.** Когда персонаж привязывается к игроку (`PlayerAttachedEvent`), он перемещается на маркер спавна своей базы. После успешного экстракта из рейда рейдер возвращается именно туда, а не на общий хаб.
- **Другой игрок = другая база.** Все данные привязаны к `NetUserId`; чужие базы недоступны физически (они на других `MapId`). Если несколько человек играют с одного аккаунта, они делят один стэш.
- **Перенос между серверами** (облако/S3) оставлен на Phase 2; сейчас прогресс живёт только в БД текущего сервера.
- **Тесты.** У режима есть интеграционные тесты (`Content.IntegrationTests/Tests/_Wega/Raid/RaidControllerTest.cs`):
  загрузка базы, телепорт на неё, вход в рейд, экстракт обратно, сохранение/загрузка стэша,
  изоляция баз разных игроков.

Подробная документация, порядок маппинга, план развития и заметки по сегодняшнему состоянию — в [`dev/RAID_MODE.md`](dev/RAID_MODE.md).
